...
significantly on Class $\pm$1 accuracy after multiplicity correction.
...
persists in matched cross-validation and comparisons of four YOLO26 checkpoints.
...
single constant $k = 1.891$, the ratio of appearances to unique bunches over
the 716 training trees, which corrects for the average number of views in which
a bunch is seen. The global divisor is calibrated only on training trees.
...
usable.\footnote{Source code: \url{https://github.com/ULM-SawitMVC/Baseline-SawitMVC}.}
...
\begin{table}[!tb]
  ...
  \begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}}lcccccc@{}}
    ...
    Global divisor    & 95.57          & 86.52          & 0.356          & ---            & ---            & ---   \\
    \addlinespace[2pt]
...
\par\vspace{3pt}
  \begin{minipage}{\columnwidth}
    \footnotesize\raggedright
    \textit{Note:} Accuracy is in \%; MAE is macro MAE. The divisor uses training GT only; --- denotes not evaluated.
  \end{minipage}
\end{table}
...
external checks. Table~\ref{tab:robust} reports two: moving the detector along
its own precision/recall curve, and swapping the checkpoint for one of a
different capacity.
...
Capacity is varied on a checkpoint-fair subset: three earlier YOLO26
checkpoints (n, s, m) trained under the previous protocol are compared with
\texttt{y26mv2} on the 64 trees outside gradient training under both protocols, with counters
fitted on the 590 trees that train under both. Across four checkpoints spanning
three model sizes, deployed Class $\pm$1 Acc moves within 3.13 points
(70.70--73.83\%), while the gap to the GT-detection condition on the very same
trees is 23.44 points. These are descriptive results within one YOLO26 family;
capacity and historical training splits are confounded, and the subset includes
detector validation trees. They neither rank architectures nor substitute for
an independent evaluation of a separately trained detector family.
...
\begin{table}[!tb]
  ...
  \renewcommand{\arraystretch}{1.13}
  \setlength{\tabcolsep}{3pt}
  \caption{Detector checkpoint and threshold sensitivity.}
  \label{tab:robust}
  \begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}}lcccc@{}}
    ...
    YOLO26n            & --- & 0.442 & 72.66 & 34.38 \\
    YOLO26s            & --- & 0.383 & 72.27 & 26.56 \\
    YOLO26m            & --- & 0.427 & 70.70 & 28.12 \\
    \texttt{y26mv2}    & --- & 0.442 & \textbf{73.83} & 31.25 \\
    GT detections      & --- & 1.000 & 97.27 & 89.06 \\
    \addlinespace[2pt]
    \multicolumn{5}{@{}l}{\emph{Operating point, Ridge$+F_{\text{all}}$, 141 test trees}} \\
    $\tau = 0.05$      & 8807 & ---   & 76.24 & 34.75 \\
    $\tau = 0.10$      & 5522 & ---   & \textbf{77.48} & \textbf{38.30} \\
    $\tau = 0.25$      & 2353 & ---   & \textbf{77.48} & 32.62 \\
    $\tau = 0.35$      & 1488 & ---   & 73.05 & 23.40 \\
    $\tau = 0.50$      &  799 & ---   & 67.91 & 21.28 \\
    $\tau = 0.70$      &  125 & ---   & 63.12 & 14.89 \\
    \bottomrule
  \end{tabular*}
\par\vspace{3pt}
  \begin{minipage}{\columnwidth}
    \footnotesize\raggedright
    \textit{Note:} Recall is macro appearance recall. The threshold sweep uses the filtered cache; recall was not recomputed for that cache.
  \end{minipage}
\end{table}
...
protocols. The detector evidence covers four checkpoints of one architecture
family, and the 64-tree comparison is descriptive rather than an architecture
ranking. A second family and a plantation excluded from detector training
remain untested. Repeated CV refits counters only; it does not estimate
detector training variability. Feature-shift tests do not establish robustness
to physical camera rotation. Test-based highlighting of configurations limits confirmatory claims;
...
Evidence remains limited to one dataset, one detector family, and two unequal
...
against learned aggregation under matched inputs, and evaluate a different
detector family. An independent plantation, calibrated pitch changes, and
