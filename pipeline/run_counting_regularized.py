"""
Regularized linear counting on 13-dim features.

Usage:
    python pipeline/run_counting_regularized.py --model ridge
    python pipeline/run_counting_regularized.py --model elasticnet
    python pipeline/run_counting_regularized.py --model ridge \
...
from build_counting_features import CLASSES, FEATURE_NAMES, load_dataset
...
def make_model(model: str) -> Pipeline:
    if model == "ridge":
        return Pipeline(
            [
...
def main() -> None:
    p = argparse.ArgumentParser(description="Regularized counting on inference features")
    p.add_argument("--model", choices=["ridge", "elasticnet"], required=True)
    p.add_argument(
        "--inference-dir",
        type=Path,
    ...
    out_dir = args.out or ROOT / "results" / "e2e_per_tree" / f"{name}_{args.model}"
    ...
    m_te["split"] = "test"
    m_te["model"] = args.model
    m_val, _ = compute_metrics(y_val, pipe.predict(X_val), ids_val)
    m_val["split"] = "val"
    m_val["model"] = args.model

    coef_source = pipe.named_steps[args.model]
    coef_df = pd.DataFrame(coef_source.coef_, index=CLASSES, columns=FEATURE_NAMES)
    ...
    label = "Ridge" if args.model == "ridge" else "ElasticNet"
    print(f"\n=== {name} - {label} (test, n={X_te.shape[0]}) ===")
