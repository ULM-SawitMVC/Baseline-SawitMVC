...
\author{%
\IEEEauthorblockN{Muhammad Zainal Muttaqin\textsuperscript{1},
Fatma Indriani\textsuperscript{1},
Setyo Wahyu Saputro\textsuperscript{1},\\
Alia Rahmi\textsuperscript{2},
Dwi Kartini\textsuperscript{1},
Triando Hamonangan Saragih\textsuperscript{1},\\
Naufal Said\textsuperscript{1},
Hartoni\textsuperscript{3}}
\IEEEauthorblockA{\textsuperscript{1}Department of Computer Science, Faculty of Mathematics and Natural Sciences\\
Lambung Mangkurat University, Banjarbaru, Indonesia}
\IEEEauthorblockA{\textsuperscript{2}Department of Agro-Industrial Technology, Faculty of Agriculture\\
Lambung Mangkurat University, Banjarbaru, Indonesia}
\IEEEauthorblockA{\textsuperscript{3}Department of Agribusiness, Faculty of Agriculture\\
Lambung Mangkurat University, Banjarbaru, Indonesia}
\IEEEauthorblockA{Email: f.indriani@ulm.ac.id}}

\maketitle

\begin{abstract}
Tree-level yield forecasting often requires a count of objects in several
...
same physical object across views. A deployed system must therefore solve two
linked problems: detecting objects in individual images and aggregating those
detections into one count vector per tree. This paper benchmarks that interaction
on 953 oil palm trees and 3,992 side-view images under two controlled detection
conditions. When the counting model receives annotated detections, it reaches
98.05\% accuracy at the class level and 92.20\% at the whole-tree level. When the
same counting task uses outputs from a fixed object detector, those accuracies
drop to 77.48\% and 32.62\%. The 20.57 percentage-point class-level gap remains
after testing multiple counting models and feature sets, indicating that detector
quality is the limiting factor in this fixed-detector pipeline rather than
tree-level count aggregation.
\end{abstract}
...
(imminent), B3 (next cycle), and B4 (future inventory)~\cite{ref12}. The
...
detectors~\cite{ref9,ref10,ref11,ref13}; these works report usable mAP on FFB
...
Recent neural-field approaches extend this idea by counting fruit from
unstructured multi-view orchard photographs in a shared 3D representation~\cite{ref8}.
...
standard front-ends for such pipelines~\cite{ref14,ref15}. In this line of work,
...
conflates two error sources. This benchmark tests that gap on SawitMVC by
...
protocol, and per-class composition are documented in the Zenodo dataset
release (doi: \url{10.5281/zenodo.20336323}).
...
All machine-learning counters are trained only on the 716-tree training split,
with deterministic seed 42; continuous outputs are rounded to the nearest
non-negative integer before metric computation. Linear Regression, Ridge,
ElasticNet, and SVM use standardized features. Ridge uses RidgeCV over
$\alpha \in \{0.01,0.1,1,10,100,500\}$, ElasticNet uses MultiTaskElasticNetCV
with 5-fold cross-validation and 3000 maximum iterations, SVM uses an RBF SVR
in a multi-output wrapper with 3-fold GridSearchCV over
$C \in \{0.1,1,10\}$ and $\gamma \in \{\text{scale},0.01,0.1\}$, and Random
Forest uses 200 trees, maximum depth 10, and random state 42.
...
ablation recovers only a small fraction of the gap. On SawitMVC under this
...
Mangkurat (ULM), Banjarmasin, Indonesia. The authors thank the field data
collection teams and annotators whose work produced the bounding box and
cross-view identity labels. The authors also thank the plantation staff for
supervising annotation and the plantation management for facilitating field
access.
...
Data: SawitMVC is available on Zenodo at
\url{https://zenodo.org/records/20336323}, doi:
\url{10.5281/zenodo.20336323}.
...
\url{10.1109/IROS60139.2025.11247341}.
